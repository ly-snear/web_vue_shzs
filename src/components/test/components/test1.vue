<template>
  <div class='table-box'>
    ...
    <div class='table-body'>
      <div v-for='(row, i) in dataList' :key='row.id' class='table-row' :data-index='i' :data-id='row.id'>
        ...
        <div class='table-columns option-box' style='flex-grow: 1;'>
          <!-- 基本信息 -->
          <!-- @dragstart.prevent='test_drop_start($event, i, row.id)' -->
          <!-- @dragenter.prevent='test_drop_enter($event, i, row.id)' -->
          <!-- @dragend='test_drop_end($event, i, row.id)' -->
          ...
          <div class='test-option'>
            <div v-for='(opt, j) in row.options' class='test-item' :key='opt.id + "_" + opt.order' :data-index='j'
    ...
    <!-- 编辑器 -->
    <Editor :pwidth='editor.width' ref='editorSingle'>
...
<script>
import Editor from './singleChooseEditor';
import { getRandomInt, numberToLetter, objectClone } from '../../../js/common/utils';
  ...
  methods: {
    ...
    loadTopicsPage() {
      ...
      data.then((res) => {
        ...
        if (!res.ok) {
          this.$Message.error(`读取单选题数据错误：${res.msg}`);
          return;
        }
        if (res.body) {
          if (res.body.data) {
    ...
    open_editor(test, index) {
      ...
      this.$refs.editorSingle.initData(true, index, test);
    },
    /**
     * 加载各类选择数据
     * 子组件调用
    ...
    new_before(test, index) {
      /**
       * 新增数据
       * @type {{chapters: string, bg: string, id_course: number, name_textbooks: string, answers: null, name_course: string, title: string, type: string, id_chapters: number, reference: number, score: number, id_term: number, name_duration: string, options: null, id_type: number, id: number, state: number, sn: number, order: number, id_duration: number, id_year: number, name_term: string, id_unit: number, name2_chapters: string, tags: null, name_unit: string, id_grade: number, grade: string, id_textbooks: number, name_year: string, id2_chapters: number, remarks: string}}
       */
      this.position = 0;
      let data = {
        id: 0,
      ...
      // console.log(test);
      this.editor.title = `正在新增单选题`;
      this.$refs.editorSingle.initData(true, index, data);
    },
    /**
     * 之后插入
     * @param test
