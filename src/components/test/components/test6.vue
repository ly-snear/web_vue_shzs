<template>
  <div class='table-box'>
    <!-- <div @click='testaddres'>测试</div>
    <div style='display: flex;width: 100%; flex-direction: row'>
      <textarea id='jeditor_describe' v-model='quest' style='width: 50%'></textarea>
      <textarea id='jeditor_describe2' v-model='quest2' style='width: 50%'></textarea>
    </div>
    <remote-script src='/static/tinymce/tinymce.min.js' @load='testadd'></remote-script> -->
    <!-- 表头 -->
    ...
    <div class='table-body'>
      <div v-for='(row, i) in dataList' :key='row.id' class='table-row' :data-index='i' :data-id='row.id'>
        <!-- 题目答案 -->
        <div class='table-columns option-box' :style='"width: calc(100% - "  + otherWidth + "px)"'>
          <!-- 基本信息 -->
          ...
          <!-- 答案列表 -->
          <div class='test-option' v-if='row.hanswer' v-html='row.hanswer'>
          </div>
        </div>
        <!-- 标记 -->
    ...
    <!-- 编辑器 -->
    <Editor :pwidth='editor.width' ref='questionsanswer'>
...
<script>
import { getRandomInt, htmlDecodeByRegExp, numberToLetter } from '../../../js/common/utils';
import '../../../js/common/importJs';
import Editor from './answerEditor';
  ...
  methods: {
    ...
    loadTopicsPage() {
      ...
      data.then((res) => {
        ...
        if (!res.ok) {
          this.$Message.error(`读取问答题数据错误：${res.msg}`);
          return;
        }
        if (res.body) {
          if (res.body.data) {
            ...
            if (this.dataList && this.dataList.length > 0) {
              for (let i = 0; i < this.dataList.length; i++) {
                ...
                this.dataList[i].htitle = htmlDecodeByRegExp(this.dataList[i].title);
                if (!this.dataList[i].answers || this.dataList[i].answers.length != 1) {
                  this.$Message.error(`读取问答题答案错误：没有提供答案`);
                  return;
                }
                this.dataList[i].hanswer = htmlDecodeByRegExp(this.dataList[i].answers[0].answer);
    ...
    open_editor(test, index) {
      ...
      this.$refs.questionsanswer.initData(true, index, test);
    },
    /**
     * 上移题目
     * @param evt
    ...
    new_before(test, index) {
      ...
      this.editor.title = `正在新增问答题`;
      this.$refs.questionsanswer.initData(true, index, data);
    },
    /**
     * 之后插入
     * @param test
    ...
    edit_success(sdata, data, index) {
      if (sdata.id > 0) {
        ...
        data.bg = sdata.bg;
        // 处理问题、答案
        data.htitle = htmlDecodeByRegExp(data.title);
        if (!data.answers || data.answers.length != 1) {
          this.$Message.error(`读取问答题答案错误：没有提供答案`);
          return;
        }
        data.hanswer = htmlDecodeByRegExp(data.answers[0].answer);
        // 重新排序题目标记
    ...
    testaddres() {
      let author_describe_data = tinymce.get('jeditor_describe').getContent();
      console.log(author_describe_data);
      console.log(this.quest);
      let author_describe_data2 = tinymce.get('jeditor_describe2').getContent();
      console.log(author_describe_data2);
