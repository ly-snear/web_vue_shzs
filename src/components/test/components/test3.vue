<template>
  <div class='table-box'>
      ...
      <div v-for='(row, i) in dataList' :key='row.id' class='table-row' :data-index='i' :data-id='row.id'>
        ...
        <div class='table-columns option-box' :style='"width: calc(100% - "  + otherWidth + "px)"' :data-index='i'
          ...
          <div class='table-columns table-body-title'
            ...
            <div class='table-body-title-text'>
              {{ row.title }}
            </div>
            <div class='table-body-title-icon'>
              <span :class='"icon iconfont " + row.icon'></span>
            </div>
          </div>
          <!-- 分值 -->
    ...
    <!-- 编辑器 -->
    <Editor :pwidth='editor.width' ref='judgetest'>
...
import Editor from './judgeEditor';
  ...
  methods: {
    ...
    loadTopicsPage() {
      ...
      data.then((res) => {
        ...
        if (!res.ok) {
          this.$Message.error(`读取判断题数据错误：${res.msg}`);
          return;
        }
        if (res.body) {
          if (res.body.data) {
            ...
            if (this.dataList && this.dataList.length > 0) {
              for (let i = 0; i < this.dataList.length; i++) {
                ...
                if (!this.dataList[i].answers || this.dataList[i].answers.length != 1) {
                  this.$Message.error(`读取判断题答案错误：没有提供答案`);
                  return;
                }
                this.dataList[i].icon = this.judgeIcon.tick;
                if (this.dataList[i].answers[0].answer != 1) {
                  this.dataList[i].icon = this.judgeIcon.error;
                }
    ...
    edit_success(sdata, data, index) {
      if (sdata.id > 0) {
        ...
        //
        if (this.dataList[index].answers[0].answer == 1) {
          this.dataList[index].icon = this.judgeIcon.tick;
        } else {
          this.dataList[index].icon = this.judgeIcon.error;
        }
        // 触发Vue响应式
        this.dataList.splice(0, 0);
      } else {
        ...
        if (this.position == 0) {
          // 之前插入
          /*
          if (data.answers[0].answer == 1) {
            data.icon = this.judgeIcon.tick;
          } else {
            data.icon = this.judgeIcon.error;
          }
          */
          if (parseInt(data.answers[0].answer) == 1) {
            data.icon = this.judgeIcon.tick;
          } else {
            data.icon = this.judgeIcon.error;
          }
    ...
    open_editor(test, index) {
      ...
      this.$refs.judgetest.initData(true, index, test);
    },
    /**
     * 上移题目
     * @param evt
    ...
    new_before(test, index) {
      ...
      this.editor.title = `正在新增判断题`;
      this.$refs.judgetest.initData(true, index, data);
    },
    /**
     * 之后插入
     * @param test
  ...
  .table-row {
    ...
    .option-box {
      ...
      .table-body-title {
        ...
        .table-body-title-text {
          flex-grow: 1;
        }

        .table-body-title-icon {
          margin-left: 5px;
          width: 32px;
        }
      }
