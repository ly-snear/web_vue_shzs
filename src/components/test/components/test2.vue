<template>
  <div class='table-box'>
    ...
    <!-- 编辑器 -->
    <Editor :pwidth='editor.width' ref='editorMultiple'>
...
<script>
import Editor from './multipleChooseEditor';
  ...
  methods: {
    ...
    loadTopicsPage() {
      ...
      data.then((res) => {
        ...
        if (!res.ok) {
          this.$Message.error(`读取多选题数据错误：${res.msg}`);
          return;
        }
        if (res.body) {
          if (res.body.data) {
    ...
    open_editor(test, index) {
      ...
      this.$refs.editorMultiple.initData(true, index, test);
    },
    /**
     * 上移题目
     * @param evt
    ...
    new_before(test, index) {
      ...
      // console.log(test);
      this.editor.title = `正在新增多选题`;
      this.$refs.editorMultiple.initData(true, index, data);
    },
    /**
     * 之后插入
     * @param test
