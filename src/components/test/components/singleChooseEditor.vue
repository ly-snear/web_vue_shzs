        ...
        <template v-if='edata.options'>
          <div v-for='(opt, index) in edata.options' :key='opt.id + "_" + opt.order' :data-index='index'
            ...
            <div class='opt_item_choice'>
              <Radio :value='opt.order' v-model='choiceValue'></Radio>
            </div>
...
export default {
  ...
  mounted() {
    /*
    EventBus.$on('selectData', (list) => {
      console.log('>>>', list);
    });
    */
  },
  destroyed() {
    // EventBus.$off('selectData');
  },
  /**
   * 组件方法
   */
  methods: {
    ...
    setCorrect() {
      if (this.edata && this.edata.options && this.edata.options.length > 0) {
        let correct = this.edata.options.find(o => o.answer == 1);
        // console.log('>>>', correct, '-->', this.edata.options);
        if (correct) {
          this.choiceValue = correct.order;
        }
      }
    },
    /**
    ...
    changeShow() {
      this.show = false;
      // let d = objectClone(this.tdata);
      // console.log('s:', this.tdata, '-->t:', d, '>>>', d == this.tdata);
     ...
     */
    changeSuccess() {
      this.$emit('changeSuccess', this.tdata, this.tindex);
    },
    // endregion
    // region 答案操作
    /**
    ...
    optionAdd(evt, opt, index) {
      ...
      // console.log(opt, newOption, this.edata.options);
    },
    /**
     * 删除答案
     * @param evt
    ...
    optionDelete(evt, opt, index) {
      this.$Confirm(`确认删除：${opt.title}`, '删除答案').then(() => {
        ...
        // console.log(opt, this.edata.options);
    ...
    tagAdd(evt, tag, index) {
      ...
      // console.log(tag, newTag, this.edata.tags);
    },
    /**
     * 删除标记
     * @param evt
    ...
    tagChange(evt, tag, index) {
      ...
      // console.log('data:', this.edata.tags);
    },
    /**
     * 新增标记
     */
    ...
    testSave() {
      ...
      if (!this.choiceValue || this.choiceValue <= 0) {
      ...
      Ajax.postJson(url, param).then((resp) => {
        if (resp.ok) {
          ...
          if (!editData || editData.id == 0) {
            ...
            // console.log(`保存题目：${param.title}，成功，但是读取数据错误--ID-->${this.tdata.id}`);
            return;
          }
          this.show = false;
          ...
          // console.log(`保存题目：${param.title}，失败-->${resp.msg}`);
        }
      });
    }
    // endregion
